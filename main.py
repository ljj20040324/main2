...
from sklearn.preprocessing import StandardScaler  # 导入特征缩放类
...
# 特征缩放
scaler = StandardScaler()  # 创建特征缩放对象
X_train = scaler.fit_transform(X_train)  # 对训练集进行特征缩放
X_test = scaler.transform(X_test)  # 对测试集进行特征缩放（使用训练集的参数）

# 创建KNN分类器
knn = KNeighborsClassifier(n_neighbors=3)  # 创建KNN分类器对象，设置邻居数为3

# 训练模型
knn.fit(X_train, y_train)  # 使用训练集训练KNN分类器

# 预测
y_pred = knn.predict(X_test)  # 使用测试集进行预测

# 打印预测结果与真实分类结果
print("KNN分类器的预测结果与真实分类结果:")
